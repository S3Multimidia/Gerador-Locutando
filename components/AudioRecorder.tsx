...
import { MicIcon, StopIcon, PlayIcon, PauseIcon, TrashIcon, LoadingSpinner } from './IconComponents';
...
export const AudioRecorder: React.FC<AudioRecorderProps> = ({ audioContext, onRecordingComplete }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center space-y-6 p-4 bg-gray-50 rounded-lg border border-gray-200 min-h-[300px]">
           {status === 'idle' && (
                <>
                    <p className="text-gray-600 text-center">Grave sua voz para usar na mixagem.</p>
                    <button onClick={handleStartRecording} className="group flex flex-col items-center justify-center w-24 h-24 bg-white rounded-full shadow-lg border-2 border-red-500 transition-all transform hover:scale-110 focus-ring">
                        <MicIcon className="w-10 h-10 text-red-500 transition-transform group-hover:scale-110" />
                    </button>
                     <p className="text-xs text-gray-500">Clique para iniciar a gravação</p>
                </>
           )}
           ...
           {(status === 'permission' || status === 'decoding') && (
               <div className="text-center space-y-4">
                  ...
                  <p className="font-semibold text-gray-700">{status === 'permission' ? 'Aguardando permissão...' : 'Processando gravação...'}</p>
           ...
           {status === 'recorded' && recordedAudio && (
               <div className="w-full flex flex-col items-center space-y-4">
                  <p className="font-semibold text-lg text-gray-800">Gravação Concluída</p>
